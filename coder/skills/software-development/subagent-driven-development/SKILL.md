...
### 1.5 Design Phase (Spec-Only Plans)

**Not all plans come with code blocks.** Some are high-level specifications describing
WHAT to build — a page redesign, a new feature, a behaviour change — without providing
exact file paths, function signatures, or code snippets. For these:

Do a design phase **in the orchestrator session** before dispatching any subagent:

1. **Research the codebase**: Read the existing files the spec references. Understand
   current patterns, data structures, imports, conventions, and how components connect.
2. **Gather data from the system**: Run DB queries (`sqlite3`, `python3 -c`), inspect
   config files, list directories, check what tables/columns exist. This is valid
   mechanical work — do it directly in the orchestrator, not via subagent.
3. **Design the exact changes**: Decide the data structures, function signatures,
   template variables, and code deltas. Write them out in your own context.
4. **Embed the design into task contexts**: Once designed, place the exact code
   blocks and file paths into each subagent's `context` parameter — just like you
   would with a copy-pasteable plan. The subagent should not need to make design
   decisions; just implement the spec as written.
5. **Then dispatch per normal SDD**: Implementer → spec reviewer → quality reviewer.
   Do NOT implement the designed code inline. The design phase output IS the
   implementation plan — dispatch it.

**TDD enforcement during design phase:** The TDD checklist MUST still be in every
implementer subagent's context — even for tasks that modify existing files rather
than creating new ones. Refactoring a route, rewriting a template, or extending an
existing function still creates new code paths that need test coverage. If the
design produces a new function (`_get_account_instructions`, new helper, new query),
it needs ≥2 tests. The orchestrator must paste the full TDD checklist into the
implementer's context, not just a one-liner saying "follow TDD."

**Signs you need a design phase:**
- Spec says "extend the route to query per-account dates" but doesn't provide the SQL
- Spec says "replace the template with cards" but doesn't provide the HTML
- Spec references a feature that requires understanding the existing database schema
- The plan file is < 50 lines of prose with no code blocks

**What NOT to do:**
- Do NOT let the subagent design the solution from scratch (they lack full codebase
  context and will make inconsistent choices across tasks)
- Do NOT implement the designed code inline (still dispatch to subagents)
- Do NOT skip spec/quality review on designed tasks (the design might have bugs too)
...
*Skip this step for **trivially verifiable** changes — see "Review granularity" under Efficiency Notes.*
...
- Dispatch multiple implementation subagents for tasks that touch the same files
  **Conversely, when tasks modify different files, parallel dispatch is a valid
  optimization** — run 2-3 implementers concurrently, then review each serially.
  This preserves the "fresh subagent per task" principle while cutting wall-clock
  time. Verify the file list before each batch to confirm no overlap.
...
- **Assume a FastAPI route parameter without `Depends()` is a dependency** — `async def my_page(user: dict | None = None)` makes `user` a **query parameter**, not an injected dependency. FastAPI treats any parameter without `Depends()` as a path/query/body parameter. If the intent is to inject the authenticated user, write `user: dict | None = Depends(get_optional_user)`. The symptom: the route always redirects to login (or returns 422) because the parameter is never populated. **Check: every route handler that accepts auth-related parameters should have `Depends()` on them.**
- **Assume `sa_text()` accepts `?` positional placeholders with a plain list** — `sqlalchemy.text()` uses `:name` style by default. Passing `["val1", "val2"]` as the second argument to `execute()` raises `ArgumentError`. Use `:a0, :a1` placeholders with `{"a0": "val1", "a1": "val2"}`. See `references/sqlalchemy-text-param-pitfalls.md`.
...
6. **Python import path sanity check** (environment quirk): After subagent does `pip install .` (not `--break-system-packages`), old pth files from bare installs cause module imports to resolve to site-packages instead of the worktree. Fix:
   ```bash
   rm -rf ~/.local/lib/python3.13/site-packages/__editable__* ~/.local/lib/python3.13/site-packages/__editable___*_finder.py ~/.local/lib/python3.13/site-packages/<package> ~/.local/lib/python3.13/site-packages/*-*.dist-info
   ...
   **When `pip install` itself fails** (not just wrong imports): If the error is
   `Could not install packages due to an OSError: No such file or directory: '.../<old-worktree-name>'`
   then pip has cached a stale resolved dependency path from a previous worktree's editable install.
   Even `--no-cache-dir` doesn't help because the stale path is embedded in the `__editable___`
   finder scripts left behind in site-packages. See `references/worktree-pip-isolation.md`
   for the full recovery sequence.

   **Relative `file:` dependency pitfall in worktrees:** When a pyproject.toml uses
   `pkg @ file:../pkg` (relative path dependencies), pip resolves this path relative to
   the worktree root, not relative to the subpackage directory inside the worktree. In a
   worktree at `.worktrees/<branch>/`, `file:../pkg` resolves to `.worktrees/pkg` instead of
   `<main-repo-root>/pkg`. This means the dependency resolution fails silently when it
   should point to a sibling package in the main repo. Workaround: temporarily use an
   absolute path in pyproject.toml (`pkg @ file:/full/absolute/path/pkg`) during
   development, then revert before committing, or pre-install the dependency package
   from its absolute path in the main repo before installing the dependent package.
...
**Data gathering — orchestrator-side, not subagent:** Running a quick DB query
(`sqlite3`, `python3 -c "select...from...""`), listing directories (`ls`, `find`),
reading AGENTS.md or README files, or checking config values are all valid
mechanical work. Do these directly in the orchestrator session to understand the
system state before designing tasks. This prevents subagents from each having to
re-discover the same facts about the environment.

**Cost trade-off:**
- More subagent invocations (implementer + 2 reviewers per task)
- But catches issues early (cheaper than debugging compounded problems later)

**Review granularity — orchestrator vs subagent:**
Not every dispatched task needs a full two-stage reviewer dispatch. Scale the review mode
to the complexity of the change:

| Change type | Review mode | Example |
|---|---|---|
| Trivially verifiable: ≤3 lines in one file, verifiable with one `grep`/syntax-check/import command | **Orchestrator verifies directly** with a single terminal command. No reviewer subagent needed. | Adding `timestamp=msg.get("timestamp"),` — verify with `grep -n "timestamp=msg.get" file.py` |
| Small change: modifies a single function or adds a small helper, but needs correctness checking | **Orchestrator runs a quick review checklist** (does it compile? does the existing test suite pass? does the change match the spec?) — one terminal command that exercises the change. | Adding a guard clause, renaming a parameter, changing a return type |
| Non-trivial: new file, new function, data transformations, multi-file changes, test additions | **Dispatch spec + quality reviewer subagents** as described in Steps 2 & 3. The subagent has the context to deeply review. | New API endpoint, new model class, data pipeline changes |

**The test suite is your ultimate reviewer.** For trivial/small changes, a passing `pytest tests/related_file.py -q` + `grep` verification is more reliable and faster than a subagent reviewer who doesn't know the codebase. Reserve full subagent review for changes where human-like judgment is needed (edge cases, security, architectural fit).

**When in doubt, dispatch the reviewer.** If you can't verify the change with a single terminal command in under 10 seconds, or if the change touches data flow, security, or shared state, use a subagent reviewer.
...
- **`references/sqlalchemy-text-param-pitfalls.md`** — SQLAlchemy `text()` defaults to named‑parameter style (`:name`), not `?` positional style. Passing a plain list to `execute()` raises `ArgumentError`. Includes the fix pattern for dynamic IN clauses. **Load during code quality review of any subagent that wrote raw SQL with `sa_text()`.**
...
- **`references/worktree-pip-isolation.md`** — `pip install -e` fails with "No such file or directory" pointing to an old worktree. Covers stale editable finder cleanup and relative `file:` dependency resolution in worktrees. **Load when pip install itself errors out, or when relative-path dependencies fail to resolve in a worktree.**

- **`references/sqlite-alter-table-migrations.md`** — SQLite ALTER TABLE ADD COLUMN pattern for existing databases when `CREATE TABLE IF NOT EXISTS` won't help. Covers `PRAGMA table_info` checks and correct ordering relative to seed INSERTs. **Load when modifying schema-creation code in a project with persistent SQLite databases.**\n\n- **`references/schema-migration-pitfalls.md`** — `CREATE TABLE IF NOT EXISTS` silently skips new columns on existing databases. Fix: ALTER TABLE ADD COLUMN with PRAGMA table_info checks, placed BEFORE any seed INSERT. Also covers `patch` tool path resolution to main repo vs worktree. **Load during code quality review of any subagent that modified schema-creation code, or when patching files inside a worktree and the edit doesn't take effect.**
