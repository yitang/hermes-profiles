...
- **Synthesise from production coding agents.** The best SOUL.md files borrow structure from Claude Code (concise updates, fix root cause), Codex (direct, friendly tone, no scope creep), and DeerFlow (task decomposition, typed plan steps). See references/system-prompts-from-other-agents.md in the coding-agent-architecture skill for full collected prompts.

### Pitfall: Hardcoding Working Directory

Do NOT hardcode terminal.cwd to an absolute path in the profile config.yaml:

```yaml
# BAD - hardcoded path
terminal:
  cwd: /Users/yitang/para/1_projects/

# GOOD - relative; resolves to where the agent was launched
terminal:
  cwd: .
```

Hardcoded paths break when the user changes their filesystem layout, when multiple users share the same profile config, or when the agent is invoked from a different project context. Let the working directory be set per-invocation (cd ~/para/1_projects && coder chat) or by the user when they start the session.
...
Skills that add no coding value -> remove to reduce noise. The token savings are minor (~800 tokens for 77->30 skills, partially offset by prompt caching), but the real reason is **skill eagerness**: some models actively try to match the user's request against the available skills list and will waste turns loading vaguely-matching skills. With 30 skills all coding-relevant, the model picks the right one on the first try.
