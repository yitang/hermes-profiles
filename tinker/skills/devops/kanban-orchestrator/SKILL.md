...
> - `kanban-profile-routing` — profile assignment, model tiering, domain-specific routing. Use to decide *which* profile gets *which* task.
> - `kanban-worker` — pitfall guide for individual workers executing tasks. You spawn workers; they load this skill automatically.
> - **Kanban Dispatcher** (daemon) — not a skill: the background process that polls boards, claims ready cards, and spawns worker processes. It auto-routes to the assigned profile.
>
