...
> - `kanban-orchestrator` — the profile that creates cards and assigns you. If your task body says "decompose this spec into tasks," load kanban-orchestrator instead of being a worker yourself.
> - `kanban-profile-routing` — the skill that defines which model/profile handles what. Your assignee name comes from here.
> - **Kanban Dispatcher** (daemon) — the background process that spawned you. You never interact with it directly; you just run until complete or blocked.
>
> Orchestrator plans, dispatcher spawns, worker executes.

> **The KANBAN_GUIDANCE block above** contains your core lifecycle (orient → work → heartbeat → block/complete). This skill adds pitfalls and examples for specific scenarios.
